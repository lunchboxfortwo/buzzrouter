...
import { registerProvisionPeerChannelsWorker } from "./jobs/provision-peer-channels";
...
try {
  ...
  await registerProvisionPeerChannelsWorker(boss, pool);
  ...
  // Kick one peer-channel pass on startup so a redeploy gives any hub
  // participant still sharing `general` its own channel immediately. Best-effort
  // for the same reasons as the other kicks above.
  try {
    await boss.send(PROVISION_PEER_CHANNELS_QUEUE, null);
  } catch (error) {
    console.error("initial hub peer channel provisioning enqueue failed", error);
  }
