...
export class ConnectorSupervisor {
  ...
  /**
   * Tell the sender, in their own channel, that nothing was delivered — once.
   *
   * ONE notice per source event, ever. Reading a source event is not a
   * once-only thing: an unrouted message never advances
   * `last_event_created_at`, the idle rebuild re-subscribes from that cursor,
   * and `since` is inclusive, so the relay replays the same unroutable event
   * for as long as it stays in range. Every other effect of reading it is
   * idempotent by source event id; this one has to be made so, or the notice
   * repeats once per rebuild forever — which is exactly what happened in
   * production, ~20 notices into a live channel at roughly one a minute.
   * {@link claimUndeliverableNotice} is that key, and it is in the database
   * rather than in memory because a restart, a redeploy or a second replica
   * would each otherwise post their own first notice.
   *
   * The claim is taken BEFORE the publish, so the failure mode is a dropped
   * notice rather than a repeated one, and a database that cannot answer stays
   ...
   *
   * The notice cannot start a loop of its own. It is signed with the bridge's
   * own key, and {@link canonicalizeSourceEvent} drops anything the bridge
   * authored before it looks at the content at all; the `br notice` tag is not
   * a projection marker, so that check is the one doing the work. As a second
   * stop the text opens with `BuzzRouter:` rather than an `@`, so even a notice
   * read by some other connector parses as unaddressed chat and routes nowhere.
   */
  private async reportUndeliverable(
    ...
    try {
      const claimed = await claimUndeliverableNotice(this.pool, {
        reason,
